...
impl<'a> Parser<'a> {
    ...
    fn parse_top_item(&mut self) -> Result<TopItem, Diag> {
        ...
        } else if self.peek_kind() == Some(&TokenKind::Kw(Kw::Let))
            || self.peek_kind() == Some(&TokenKind::Kw(Kw::Mut))
        {
    ...
    fn parse_type_decl(&mut self) -> Result<TypeDecl, Diag> {
        ...
        let kind = if self.advance_if_op("=") {
            ...
            loop {
                ...
                if self.advance_if_op("(") && !self.advance_if_op(")") {
                    loop {
    ...
    fn parse_type_expr(&mut self) -> Result<TypeExpr, Diag> {
        let t = self
            .peek()
            .ok_or_else(|| self.err::<()>("type expression").unwrap_err())?
            .clone();
        let span = Span {
            line: t.line,
            col: t.col,
    ...
    fn parse_block(&mut self) -> Result<Block, Diag> {
        let t = self
            .expect(&TokenKind::Newline, "newline before block")?
            .clone();
        let span = Span {
            line: t.line,
            col: t.col,
    ...
    fn parse_statement(&mut self) -> Result<Statement, Diag> {
        if self.peek_kind() == Some(&TokenKind::Kw(Kw::Let))
            || self.peek_kind() == Some(&TokenKind::Kw(Kw::Mut))
        {
    ...
    fn parse_pattern(&mut self) -> Result<Pattern, Diag> {
        let t = self
            .peek()
            .ok_or_else(|| self.err::<()>("pattern").unwrap_err())?
            .clone();
        let span = Span {
            line: t.line,
            col: t.col,
    ...
    fn parse_cmp_expr(&mut self) -> Result<Expr, Diag> {
        ...
        if let Some(bin_op) = op {
            ...
            Ok(Expr {
                span: left.span.clone(),
                kind: ExprKind::Binary(bin_op, Box::new(left), Box::new(right)),
            })
        } else {
            Ok(left)
        }
    ...
    fn parse_range_expr(&mut self) -> Result<Expr, Diag> {
        ...
        if let Some(bin_op) = op {
            ...
            let right = if let Some(TokenKind::Op(o)) = self.peek_kind() {
                ...
                None
            } else if let Some(TokenKind::Kw(
                Kw::If | Kw::While | Kw::For | Kw::In | Kw::Return | Kw::Else | Kw::Elif,
            )) = self.peek_kind()
            {
                // End of expression
                None
            ...
            if let Some(r) = right {
                Ok(Expr {
                    span: left.span.clone(),
                    kind: ExprKind::Binary(bin_op, Box::new(left), Box::new(r)),
                })
            } else {
                ...
                Ok(Expr {
                    span: left.span.clone(),
                    kind: ExprKind::Binary(bin_op, Box::new(left), Box::new(none_expr)),
                })
            }
        } else {
            Ok(left)
    ...
    fn parse_unary_expr(&mut self) -> Result<Expr, Diag> {
        let t = self
            .peek()
            .ok_or_else(|| self.err::<()>("expression").unwrap_err())?
            .clone();
        let span = Span {
            line: t.line,
            col: t.col,
        ...
        if let Some(un_op) = op {
            Ok(Expr {
                span,
                kind: ExprKind::Unary(un_op, Box::new(inner)),
            })
        } else {
            Ok(inner)
        }
            ...
            if self.advance_if_op(".") {
                ...
                if self.peek_kind() == Some(&TokenKind::Op("(")) {
                    ...
                    expr = Expr {
                        ...
                        kind: ExprKind::Call(Box::new(field_expr), args),
                    };
                } else {
                    expr = Expr {
                        span: expr.span.clone(),
    ...
    fn parse_primary(&mut self) -> Result<Expr, Diag> {
        ...
        if let Some(TokenKind::Str(parts)) = self.peek_kind() {
            ...
            return Ok(Expr {
                span,
                kind: ExprKind::InterpStr(interp),
            });
        }
        ...
        if let Some(TokenKind::Ident(i)) = self.peek_kind() {
            ...
            if self.advance_if_op("{") {
                ...
                return Ok(Expr {
                    span,
                    kind: ExprKind::Record(id, fields),
                });
            }
            return Ok(Expr {
                span,
        ...
        if self.advance_if_kw(Kw::Fn) {
            ...
            return Ok(Expr {
                span,
                kind: ExprKind::Closure(params, ret_type, body),
            });
        }
        ...
        if self.advance_if_kw(Kw::Try) {
            ...
            return Ok(Expr {
                span,
                kind: ExprKind::Try(Box::new(expr), else_exit),
            });
        }
